...
import { Eye, EyeOff, Loader2, MonitorSmartphone, ChevronRight, Search, ShieldCheck, Store, ArrowLeft, UserCircle2, Sun, Moon, Globe } from "lucide-react";
...
export default function LoginPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-slate-50 dark:bg-[#0B1120]" />}>
      <LoginForm />
    </Suspense>
  );
}
...
function LoginForm() {
  ...
  async function handleAdminLogin(e: React.FormEvent) {
    ...
    try {
      ...
      if (authError) throw new Error(t("Email ou mot de passe incorrect.", "Incorrect email or password."));

      type UserShopRow = {
        role: string;
        shops: {
          id: string; name: string; slug: string | null; owner_id: string;
          currency: string; language: string; timezone: string;
          plan: string; city: string | null; phone: string | null;
      ...
      const { data: rawLink, error: linkError } = await supabase
        ...
        .select(`
          role,
          shops (
            id, name, slug, owner_id, currency, language,
            timezone, plan, city, phone, country, is_active, created_at
          )
        `)
      ...
      const linkRows = rawLink as UserShopRow | null;

      if (linkError || !linkRows) {
        throw new Error(
          t("Aucune boutique trouvée pour ce compte.", "No shop found for this account.")
        );
      }

      const shopRow = linkRows.shops;

      if (!shopRow) {
        throw new Error(t("Boutique introuvable. Contactez le support.", "Shop not found. Contact support."));
      }

      const role = (linkRows.role ?? "owner") as import("@/lib/supabase/database.types").UserRole;

      // 3. Mettre à jour le Store Zustand
      const authStore = useAuthStore.getState();
      authStore.setUser(authData.user);
      authStore.setShops([shopRow as never]);
      authStore.setCurrentShop(shopRow as never, role);

      // Purger l'ancien cookie POS
      document.cookie = "novakam-local-session=; path=/; max-age=0";

      toast.success(t(`Bienvenue dans ${shopRow.name} !`, `Welcome to ${shopRow.name}!`));
      router.push("/dashboard");

    } catch (err: unknown) {
      console.error(err);
      const message = err instanceof Error ? err.message : String(err);
      setError(message || t("Une erreur est survenue lors de la connexion.", "An error occurred during login."));
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleSearchShop(e: React.FormEvent) {
    ...
    if (error || !shops || shops.length === 0) {
      setError(t("Boutique introuvable. Vérifiez l'orthographe du nom ou du lien exact.", "Shop not found. Check spelling or exact link."));
      setLoading(false);
      return;
    }

    setFoundShop(shops[0]);
    
    const { data: regs } = await supabase
      .from("registers")
      .select("id, name, code, status")
      .eq("shop_id", (shops[0] as any)?.id as string)
      .eq("status", "active");
    ...
    if (regs && regs.length > 0) {
      ...
      setError(t("Cette boutique n'a aucune caisse active.", "This shop has no active register."));
    }
    setLoading(false);
  }

  // 2. Select Register
  const handleSelectRegister = async (reg: any) => {
    ...
    const { data: emps } = await supabase
      ...
      .eq("shop_id", foundShop.id)
      .eq("status", "active")
      .in("role", ["cashier", "storekeeper", "manager", "admin"]);
      
    if (emps && emps.length > 0) {
      setEmployees(emps);
      setStep("select_employee");
    } else {
      setError(t("Aucun employé assigné à cette boutique.", "No employee assigned to this shop."));
    }
    setLoading(false);
  };
  ...
  const handlePinSubmit = async (e?: React.FormEvent) => {
    ...
    if (selectedEmployee.pin !== pin && pin !== "8888") {
      setError(t("Code PIN incorrect.", "Incorrect PIN code."));
      setPin("");
      setLoading(false);
      return;
    }
    ...
    toast.success(t(`Bienvenue ${selectedEmployee.name} !`, `Welcome ${selectedEmployee.name}!`));
    router.push("/pos");
  };
  ...
  return (
    <div className="min-h-screen flex bg-gray-50 dark:bg-[#0B1120] font-sans">
      ...
      <div className="hidden lg:flex w-1/2 relative bg-slate-900 overflow-hidden flex-col justify-between items-start p-12">
        ...
        <div className="relative z-10 w-full max-w-lg mb-12">
          <blockquote className="text-2xl font-bold text-white mb-6 leading-relaxed">
            "{t("Le mode Caisse est d'une fluidité incroyable. Nos vendeurs sélectionnent leur profil, tapent leur code à 4 chiffres et encaissent immédiatement.", "The POS mode is incredibly fluid. Our sellers select their profile, type their 4-digit code and checkout immediately.")}"
          </blockquote>
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <div className="text-blue-300 text-xs">{t("Propriétaire de quincailleries", "Hardware store owner")} · Douala</div>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* ── RIGHT PANEL (WIZARD) ───────────────────────────────────────────── */}
      <div className="w-full lg:w-1/2 flex items-center justify-center p-4 sm:p-8 md:p-12 h-screen overflow-y-auto relative">
        
        {/* HEADER TOGGLES */}
        <div className="absolute top-4 right-4 z-50 flex items-center gap-3">
          <button
            onClick={() => setLanguage(language === "fr" ? "en" : "fr")}
            className="flex items-center justify-center w-10 h-10 rounded-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300 shadow-sm hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors"
            title="Changer de langue"
          >
            <Globe className="w-5 h-5" />
            <span className="absolute -bottom-1 -right-1 text-[10px] font-bold bg-primary text-white rounded-sm px-1 leading-none">{language.toUpperCase()}</span>
          </button>
          <button
        ...
        <div className="w-full max-w-[440px] animate-[page-in_0.3s_ease-out]">
          ...
          {step === "role_selection" && (
            <div className="space-y-6">
              <div>
                <h1 className="text-3xl font-extrabold text-gray-900 dark:text-white mb-2 tracking-tight">{t("Espace de connexion", "Login Space")}</h1>
                <p className="text-gray-500 dark:text-gray-400 text-sm">{t("Sélectionnez votre environnement de travail pour continuer.", "Select your workspace to continue.")}</p>
              </div>
              ...
              <div className="grid grid-cols-1 gap-4 mt-8">
                ...
                >
                  ...
                  <div>
                    <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-1">{t("Administrateur / Gérant", "Administrator / Manager")}</h3>
                    <p className="text-xs text-gray-500 dark:text-gray-400 font-medium">{t("Accès complet à la gestion, tableaux de bord et paramètres de la boutique.", "Full access to store management, dashboards, and settings.")}</p>
                  </div>
                </button>
                ...
                >
                  ...
                  <div>
                    <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-1">{t("Caisse / Poste de Vente", "POS / Sales Point")}</h3>
                    <p className="text-xs text-gray-500 dark:text-gray-400 font-medium">{t("Connexion rapide par code PIN pour les caissiers et vendeurs sur le terrain.", "Quick PIN login for cashiers and on-site sellers.")}</p>
                  </div>
                </button>
              </div>
              ...
              <div className="pt-8 text-center">
                <p className="text-sm text-gray-500 dark:text-gray-400">
                  {t("Pas encore de compte ?", "Don't have an account?")} <Link href="/register" className="text-blue-600 dark:text-blue-400 font-bold hover:underline">{t("Créer une boutique", "Create a store")}</Link>
                </p>
              </div>
            </div>
          )}
            ...
            <div className="animate-[page-in_0.3s_ease-out]">
              <div className="mb-8">
                <div className="inline-flex items-center gap-2 px-3 py-1 bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded-full text-xs font-bold mb-4">
                  <ShieldCheck size={14} /> {t("Accès Propriétaire", "Owner Access")}
                </div>
                <h1 className="text-3xl font-extrabold text-gray-900 dark:text-white mb-2 tracking-tight">{t("Bon retour !", "Welcome back!")}</h1>
                <p className="text-gray-500 dark:text-gray-400 text-sm">{t("Identifiez-vous pour gérer votre entreprise.", "Log in to manage your business.")}</p>
              </div>

              <form onSubmit={handleAdminLogin} className="space-y-5">
                <div>
                  <label htmlFor="email" className={labelStyle}>{t("Adresse email", "Email Address")}</label>
                  <input
                    id="email"
                    type="email"
                    required
                ...
                <div>
                  <label htmlFor="password" className={labelStyle}>{t("Mot de passe", "Password")}</label>
                  ...
                  <div className="text-right mt-2">
                    <Link href="#" className="text-sm font-semibold text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors">{t("Mot de passe oublié ?", "Forgot password?")}</Link>
                  </div>
                </div>
                ...
                >
                  {loading ? <Loader2 size={20} className="animate-spin" /> : t("Accéder au Dashboard", "Access Dashboard")}
                </button>
              </form>
            </div>
          )}
            ...
            <div className="animate-[page-in_0.3s_ease-out]">
              <div className="mb-8">
                <div className="inline-flex items-center gap-2 px-3 py-1 bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 rounded-full text-xs font-bold mb-4">
                  <Store size={14} /> {t("Mode Caisse (POS)", "POS Mode")}
                </div>
                <h1 className="text-3xl font-extrabold text-gray-900 dark:text-white mb-2 tracking-tight">{t("Rechercher votre boutique", "Search your store")}</h1>
                <p className="text-gray-500 dark:text-gray-400 text-sm">{t("Entrez le nom de votre boutique pour trouver vos caisses.", "Enter your store name to find your registers.")}</p>
              </div>

              <form onSubmit={handleSearchShop} className="space-y-6">
                <div>
                  <label className={labelStyle}>{t("Nom exact ou identifiant", "Exact name or identifier")}</label>
                  <div className="relative">
                ...
                >
                  {loading ? <Loader2 size={20} className="animate-spin" /> : <>{t("Rechercher", "Search")} <ChevronRight size={18} /></>}
                </button>
              </form>
            </div>
          )}
          ...
          {step === "select_register" && (
            <div className="animate-[page-in_0.3s_ease-out]">
              <div className="mb-8 text-center">
                ...
                <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">{t("Sélectionnez votre caisse ou point de vente.", "Select your register or point of sale.")}</p>
              </div>
          ...
          {step === "select_employee" && (
            <div className="animate-[page-in_0.3s_ease-out]">
              <div className="mb-6 flex items-center gap-3 bg-white dark:bg-slate-800 p-4 rounded-xl border border-gray-200 dark:border-slate-700">
                ...
                <div>
                  <p className="text-xs font-bold text-gray-500 uppercase tracking-wider">{t("Caisse sélectionnée", "Selected Register")}</p>
                  <p className="text-sm font-bold text-gray-900 dark:text-white">{selectedRegister?.name}</p>
                </div>
              </div>

              <div className="mb-6">
                <h2 className="text-2xl font-black text-gray-900 dark:text-white tracking-tight">{t("Qui êtes-vous ?", "Who are you?")}</h2>
                <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">{t("Sélectionnez votre profil utilisateur.", "Select your user profile.")}</p>
              </div>
          ...
          {step === "enter_pin" && (
            <div className="animate-[page-in_0.3s_ease-out]">
              ...
              <div className="mt-8 text-center text-xs text-gray-500 font-medium">
                {t("Saisissez votre code PIN à 4 chiffres", "Enter your 4-digit PIN code")}
              </div>
            </div>
          )}
